...
const status = <T>(
  ...
  if (error instanceof HttpErrorResponse && status.includes(error.status) && error.headers.has("X-Compass-Host")) {
...
export const badRequestOrConflict = <T>(
  fn: (model: ErrorResponseModel, response: HttpErrorResponse) => Promise<T> | T
): ((error: unknown) => Promise<T>) =>

  status([ HttpStatusCode.BadRequest, HttpStatusCode.Conflict ], fn);
